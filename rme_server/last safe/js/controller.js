var tableMethods = {
	...
	loadContracts: function () {
		...
		if (! contracts) {
			$.getJSON('http://alexsaw.com/rme/js/contracts.json', function (data) {
	...
	loadCustomers: function () {
		...
		if (! customers) {
			$.getJSON('http://alexsaw.com/rme/js/customers.json', function (data) {
	...
	loadMassUpdates: function () {
		...
		if (! massUpdates) {
			...
			$.getJSON('http://alexsaw.com/rme/js/medicaid-mass-updates.json', function (data) {
	...
	loadPrograms: function () {
		...
		if (! programs) {
			...
			$.getJSON('http://alexsaw.com/rme/js/medicaid-programs.json', function (data) {
				sessionStorage.setItem('programs', JSON.stringify( data ));
			}).done(function () {
				var programs = JSON.parse(sessionStorage.getItem('programs'));
				for (var i = programs.length - 1; i >= 0; i--) {
					if(i == 78){
						$('.programTable table').append("<tr class='disabledProgram'><td data-id='"+programs[i].id+"' class='check'></td><td>"+programs[i].shortName+"</td><td>"+programs[i].name+"</td><td>"+programs[i].type+"</td><td>"+programs[i].start+"</td><td>"+programs[i].end+"</td></tr>")
					}else{
						$('.programTable table').append("<tr><td data-id='"+programs[i].id+"' class='check'><input class='checkOne' type='checkbox'></td><td>"+programs[i].shortName+"</td><td>"+programs[i].name+"</td><td>"+programs[i].type+"</td><td>"+programs[i].start+"</td><td>"+programs[i].end+"</td></tr>")
					}
	...
	loadProducts: function () {
		...
		if (! products) {
			$.getJSON('http://alexsaw.com/rme/js/medicaid-products.json', function (data) {
