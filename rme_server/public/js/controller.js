	...
	loadMassUpdates: function () {
		...
		if (! massUpdates) {
			...
			$.getJSON('../js/medicaid-mass-updates.json', function (data) {
				...
				$('.massUpdateTable table').append("<tr><td class='id massUpdateId'>"+massUpdates[i].id+"</td><td>"+massUpdates[i].name+"</td><td>"+massUpdates[i].status+"</td><td>"+massUpdates[i].originator+"</td><td>"+massUpdates[i].approver+"</td><td>"+massUpdates[i].edited+"</td><td>"+massUpdates[i].implemented+"</td></tr>")
				};
			});
		}
		else {
		...
		var programs = JSON.parse(sessionStorage.getItem('programs'));
			...
			$.getJSON('../js/medicaid-programs.json', function (data) {
				...
				for (var i = programs.length - 1; i >= 0; i--) {
					...
					$('.programsToAddTable table').append("<tr class='id' data-id="+programs[i].id+"><td class='check'><input class='checkOne' type='checkbox'></td><td>"+programs[i].name+"</td><td>"+programs[i].type+"</td><td>"+programs[i].start+"</td><td>"+programs[i].end+"</td></tr>");

				};
			});
	},

	loadSelectedPrograms: function () {
		var selectedPrograms = JSON.parse(sessionStorage.getItem('selectedPrograms'));

		if(selectedPrograms != 0 && selectedPrograms.length){
			for (var i = selectedPrograms.length - 1; i >= 0; i--) {
				$('.programsToAddTable table').append("<tr class='id' data-id="+selectedPrograms[i].id+"><td class='check'><input class='checkOne' type='checkbox'></td><td>"+selectedPrograms[i].name+"</td><td>"+selectedPrograms[i].type+"</td><td>"+selectedPrograms[i].start+"</td><td>"+selectedPrograms[i].end+"</td></tr>");
			}
		}
	},
