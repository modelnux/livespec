...
var configSummaryPopover = function () {
	...
	$button.on("click", function (e) {
			...
			if ($popover.is(":visible")) {
				...
				var $target = $button;	
				var	left = $target.offset().left - 234
		  ...
		  function commaSeparateNumber(val){
			...
			var currentTotal = parseInt($('.total-price').text().replace('$','').replace(',',''));
				...
				if ($(this).closest('.right-panel-section').find('.check-icon').length) {
					...
					li.children('.section-item-alert').hide();
					...
					li.children('.section-item-alert').show();
					li.children('.check-icon').remove();
				}

				if($('.left-panel').find('.section-item-alert:visible').length == 0) {
					$('.update-quote').removeClass('disabled');
				}
				else {
					$('.update-quote').addClass('disabled');
				}
